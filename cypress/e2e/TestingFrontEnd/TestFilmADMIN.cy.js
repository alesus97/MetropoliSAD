describe("Test film ADMIN", () => {
  ...
  it("Inserimento nuovo film", function () {
    ...
    cy.get('[name="locandina"]').type(
      "https://pad.mymovies.it/filmclub/2005/05/058/locandina.jpg"
    );
    cy.get('[data-cy="ok"]')
      .click()
      .then(() => {
        ...
        cy.get(
          '[data-cy="filmCard-La fabbrica di cioccolato-Brad Grey, Richard D. Zanuck"]'
        ).should("be.visible");
      });
  });
  ...
  it("Cancellazione film", function () {
    ...
    cy.get(
      '[data-cy="filmCard-La fabbrica di cioccolato-Brad Grey, Richard D. Zanuck"] path'
    ).click();
  ...
  it("Inserimento film già esistente", function () {
    ...
    cy.get('[data-cy="genere"]').type(
      "noir, gangaster, commedia, thriller, drammatico"
    );
    ...
    cy.get('[data-cy="trama"]').type(
      "I destini di un killer, della sua compagna e di un pugile in declino s'intrecciano in questa cruenta pellicola"
    );
    cy.get('[name="locandina"]').type(
    ...
    cy.get('[data-cy="ok"]')
      ...
      .then(() => {
        cy.get('[data-cy="AlertDialog"]').contains(
          "Duplicate entry 'Lawrence Bender-Pulp Fiction' for key 'film.produttore_titolo_unique'"
        );
      });
  });
});
