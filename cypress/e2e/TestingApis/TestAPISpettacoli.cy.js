...
before(async ()=>{
    ...
    it('GET', async function() { 
        ...
        APIService.createSpettacolo(181,spettacolo).then((res)=>{
           // console.log(res) 
            expect(res.status===200 || res.data.message === "Spettacolo inserito correttamente").to.be.true
           // console.log(res)
             onDeleteIndex = res.data.codice_spettacolo
           // console.log(onDeleteIndex)   
              APIService.deleteCinema(onDeleteIndex).then((res)=>{
               expect(res.status===200 || res.data.message === "Il cinema è stato correttamente eliminato.").to.be.true
                     console.log(res)
