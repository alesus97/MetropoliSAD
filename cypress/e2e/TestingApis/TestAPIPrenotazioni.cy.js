...
before(async ()=>{
    ...
    describe('PRENOTAZIONE', ()=>{
         ...
         it('GET BIGLIETTI', async function() { 
            APIService.getBiglietti().then((res)=>{
        ...
        it('GET POSTI DISPONIBILI - CREATE BIGLIETTI', async function() { 
            ...
            APIService.getPostiDisponibili(234).then((resPosti)=>{
                 ...
                 posti.codici_posto.push(posto.codice_posto)
                  APIService.createBiglietto(posti, 234).then((res)=>{
                      expect(res.status===200 || res.data.message === "biglietti registrati correttamente").to.be.true
                      console.log(res)
